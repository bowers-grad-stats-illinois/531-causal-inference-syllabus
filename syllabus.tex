...
For nearly every problem we encounter, methodologists across disciplines ---
econometricians, statisticians, political methodologists, epidemiologists,
and others --- have developed more than one credible approach. Some
approaches estimate average causal effects defined in terms of
counterfactual outcomes; others test hypotheses about those effects without
committing to a point estimate. Some ground their inferences in models for
the treatment assignment process (how units came to be treated); others
ground them in models for the outcome process (how outcomes would have
evolved absent treatment). Some derive their statistical properties from
the randomization that produced the data; others derive them from
assumptions about sampling from a larger population. These are not
competing schools where one is right and the rest are wrong. They are
complementary tools that rest on different assumptions and answer subtly
different questions. Throughout this course we will try to present at
least two approaches to each topic, so that you develop the judgment to
choose --- and to explain your choice --- when confronting a causal
question in your own work.
...
\end{itemize}
    ...
    \begin{enumerate}
        ...
        \item 	All papers written in this class will assume familiarity with
            the principles of good writing in \fullcite{becker:1986}.
    ...
    I'll calculate your grade for the course this way: 40\% explorations (when
    ...
    concepts, no late work accepted); 45\% final project (graded using a rubric
    to help you not forget important topics); 15\% attendance (satisfactory if you show up, fail if not).
...
\begin{itemize}[leftmargin=*]
  ...
  \item The \textbf{Exploration} for this week focuses on two of the main concepts of causal inference arising from DAGs.
\end{itemize}
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \fullcite{morganwinship2015}, Ch. 3.1--3.2, 3.4
...
\textbf{Recommended:}
\begin{enumerate}[noitemsep,leftmargin=*]
  \item \fullcite{waldner2025book} Ch. 1; Ch. 2 up through \S 2.2.2 on DAGs
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \fullcite{lewbel2019identification}
\end{enumerate}
\textbf{Recommended:}
\begin{enumerate}[noitemsep,leftmargin=*]
...
\begin{enumerate}[noitemsep,leftmargin=*]
    ...
    \item \fullcite{leavitt2026matching}
...
\weekheading{From Counterparts to Difference-in-Differences: Assumptions About Outcomes}
\begin{itemize}[leftmargin=*]
  \item The shift from assignment assumptions to outcome assumptions: what if we cannot control or exploit the treatment assignment mechanism?
  \item Rosenbaum's counterparts: using analogous comparisons (not true controls) to reinforce causal claims --- and their limitations, including the log transformation problem
  \item The parallel trends assumption and the ATT estimand
  \item DiD by hand, via TWFE regression, and via matched designs
  \item Bracketing: DiD vs.\ lagged outcome adjustment as bounds on the ATT
  \item Risk-set matching for panel data: building the matched comparison by hand (with \texttt{optmatch}/\texttt{designmatch}) and via \texttt{PanelMatch}
  \item Pre-trends diagnostics and their limitations
  \item Sensitivity analysis: how much violation of parallel trends would be needed to change the conclusion? (Design-based and econometric approaches)
\end{itemize}
\textbf{Required:}
\begin{enumerate}[noitemsep,leftmargin=*]
    \item \fullcite{rosenbaum2017}, Ch.\ 8 (pp.\ 154--166 on counterparts and difference-in-differences)
    \item \fullcite{rothetal2023}, Secs.\ 1--2
    \item \fullcite{cohnsongzubizarreta2025}
\end{enumerate}
\textbf{Recommended:}
\begin{enumerate}[noitemsep,leftmargin=*]
    \item \fullcite{rosenbaum2001b}
    \item \fullcite{cunningham2021causal}, Ch.\ 9
    \item \fullcite{dingli2019}
    \item \fullcite{keelesmallhsufogarty2019}
    \item \fullcite{rambachanroth2023}
    \item \fullcite{leavitthatfield2025}
    \item \fullcite{leavitt2025pretrends}
    \item \fullcite{imaikim2019}
    \item \fullcite{rauhkimimai2025}
    \item \url{https://diff.healthpolicydatascience.org/} (Leavitt, Zeldow \& Hatfield)
\end{enumerate}

\weekheading{Staggered Treatment Timing and the Anatomy of Event Studies}
\begin{itemize}[leftmargin=*]
    \item The TWFE problem under staggered adoption: Goodman-Bacon decomposition, forbidden comparisons, negative weighting
    \item Why TWFE uses the wrong counterparts: already-treated units are contaminated comparisons
    \item Modern estimators: Callaway \& Sant'Anna, imputation approaches, de Chaisemartin \& d'Haultf\oe uille
    \item The experimental perspective on event studies: which observations matter, information borrowing, diagnostics (Shen et al.)
    \item Design-based inference for DiD with staggered adoption (Athey \& Imbens)
    \item Risk-set matching in staggered settings: constructing proper counterpart groups at each adoption time
\end{itemize}
\textbf{Required:}
\begin{enumerate}[noitemsep,leftmargin=*]
    \item \fullcite{bakeretal2025}
    \item \fullcite{shen2024anatomy}, Secs.\ 1--3
    \item \fullcite{atheyimbens2022}
\end{enumerate}
\textbf{Recommended:}
\begin{enumerate}[noitemsep,leftmargin=*]
    \item \fullcite{rothetal2023}, Secs.\ 3--4
    \item \fullcite{goodman-bacon2018b}
    \item \fullcite{callawaysantanna2019}
    \item \fullcite{cunningham2021causal}, Ch.\ 9
    \item \fullcite{chattopadhyayzubizarreta2023}
    \item \fullcite{cohnsongzubizarreta2025} (if not read in Week 11)
\end{enumerate}

\weekheading{Synthetic Controls and the Hierarchy of Panel Methods}
\begin{itemize}[leftmargin=*]
    \item Synthetic controls as optimized counterpart construction: the donor pool as candidate counterparts, optimization finds the best composite
    \item The method concretely: California Proposition 99 (Abadie, Diamond \& Hainmueller 2010)
    \item Inference via placebo tests as permutation inference: which unit ``received'' the policy is the randomization
    \item Augmented synthetic controls: bias correction when pre-treatment fit is imperfect (Ben-Michael, Feller \& Rothstein)
    \item Synthetic DiD: bridging DiD and synthetic controls (Arkhangelsky et al.)
    \item The hierarchy of panel methods: DiD $\to$ matched DiD $\to$ synthetic control $\to$ augmented SC $\to$ matrix completion --- assumptions, weights, and inferential frameworks
\end{itemize}
\textbf{Required:}
\begin{enumerate}[noitemsep,leftmargin=*]
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \fullcite{arkhangelsky2021}
  \item \fullcite{cunningham2021causal}, synthetic control chapter
  \item \fullcite{rosenbaum2015a}
\end{enumerate}

\weekheading{Mediators, Front-door Paths, and Post-treatment Variables}
\begin{itemize}[leftmargin=*]
  \item The danger of conditioning on post-treatment variables: why ``controlling for'' a mediator does not recover direct or indirect effects without strong assumptions, even in randomized experiments
  \item Rosenbaum's design-based alternatives: tightened blocks for affected concomitant variables, known/unaffected outcomes as tests for bias, and differential effects to remove generic unmeasured biases
  \item Causal mediation analysis (Imai, Keele, Tingley, Yamamoto): the ACME and ADE estimands, sequential ignorability, the \texttt{mediation} R package, and sensitivity analysis
  \item The Green/Bullock critique: mediation analysis requires assumptions strong enough to undermine the experimental advantage --- design experiments that test mechanisms directly instead
  \item Front-door adjustment (Pearl, Glynn): identifying total effects by routing through a complete mediator when treatment--outcome confounding is unmeasured
  \item Comparing assumptions: sequential ignorability allows direct effects but requires no unmeasured confounding anywhere; front-door tolerates unmeasured treatment--outcome confounding but requires complete mediation
\end{itemize}
\textbf{Required:}
\begin{enumerate}[noitemsep,leftmargin=*]
  \item \fullcite{rosenbaum2025}, Ch.\ 14 (pp.\ 337--359 on tightened blocks, affected concomitants, and differential effects)
  \item \fullcite{imaietal2011}
  \item \fullcite{bullocketal2010}
  \item \fullcite{bellemare2024frontdoor}
\end{enumerate}
\textbf{Recommended:}
\begin{enumerate}[noitemsep,leftmargin=*]
  \item \fullcite{rosenbaum2025}, Ch.\ 12, Secs.\ 12.1--12.2 (known effects as bias diagnostics)
  \item \fullcite{rosenbaum1984c}
  \item \fullcite{rosenbaum2017}, Ch.\ 7, pp.\ 118--126 (mechanisms and elaborate theories)
  \item \fullcite{imaietal2010}
  \item \fullcite{tingleyetal2014}
  \item \fullcite{greenetal2010}
  \item \fullcite{glynn2018front}
  \item \fullcite{glynnkashin2017}
  \item \fullcite{montgomeryetal2018}
  \item \fullcite{acharyaetal2016c}
  \item \fullcite{frangakisrubin2002}
  \item \fullcite{robinsgreenland1992}
\end{enumerate}

\weekheading{Review Week focusing on Regression}
\textbf{Presentations by the class on:}
\begin{enumerate}[leftmargin=*]
  \item Regression is not research design
  \item Regression for description
  \item Regression for statistical inference
  \item Regression for causal inference
\end{enumerate}
...
\pdfbookmark[2]{Final Projects Due: 18 May 2026}{finalprojectsdue}
\subsection*{Final Projects Due --- Monday, 18 May 2026}
...
\begin{itemize}
    ...
    \item Proximal causal inference and negative controls: when a researcher has
        two proxy variables for an unmeasured confounder --- one associated with
        treatment assignment and one associated with the outcome --- these can
        identify causal effects without observing the confounder itself, using a
        two-stage procedure analogous to instrumental variables.
        \begin{enumerate}[noitemsep]
          \item \fullcite{tchetgenetal2024proximal}
          \item \fullcite{miaogengtchetgen2018}
          \item \fullcite{lipsitchetal2010}
          \item \fullcite{soferetal2016}
        \end{enumerate}
    \item Other estimands: Average interference effects on networks and graphs
    ...
    \item Predictive Bayesian causal inference and machine learning.\\
        ...
        \end{enumerate}
    \item Causal inference via process tracing (\fullcite{waldner2025book})
\end{itemize}
